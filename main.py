...
def get_assistant_ids():
    db = load_db()
    return [a["id"] for a in db.get("assistant_admins", [])]


def is_support_staff(uid):
    """يرجع True للأدمن الرئيسي أو أي مساعد مُضاف بصلاحية الرد على الدعم."""
    return uid == ADMIN_ID or uid in get_assistant_ids()
...
def localized_field(container, field, lang, translate_fn=None):
    ...
    يخزّن أيضاً نسخة من النص العربي الأصلي وقت الترجمة (field + '_en_src')،
    وإذا تغيّر النص العربي لاحقاً (تعديل الأدمن له) تُعاد الترجمة تلقائياً بدل
    عرض ترجمة قديمة لا تطابق النص الحالي.
    ...
    src_key = f"{field}_en_src"
    cached = container.get(cache_key)
    cached_src = container.get(src_key)
    if cached and cached_src == original:
        return cached, False
    translated = (translate_fn or translate_text)(original, "en")
    container[cache_key] = translated
    container[src_key] = original
...
WAIT_BTN_SEARCH = "WAIT_BTN_SEARCH"
WAIT_BROADCAST_SCHEDULE_TIME = "WAIT_BROADCAST_SCHEDULE_TIME"
WAIT_INACTIVITY_DAYS = "WAIT_INACTIVITY_DAYS"
WAIT_INACTIVITY_TEXT = "WAIT_INACTIVITY_TEXT"
WAIT_ASSISTANT_ADD_ID = "WAIT_ASSISTANT_ADD_ID"
WAIT_ASSISTANT_REMOVE_ID = "WAIT_ASSISTANT_REMOVE_ID"
...
DEFAULT_CONFIG = {
    ...
    "inactivity_days": 7,
    "inactivity_bonus_points": 1,
    "inactivity_text": "😴 وينك؟ زعلنا من غيابك! تعال ولك نقطة هدية بانتظارك 🎁",
    "inactivity_last_run_date": None,
...
def log_points_change(user_rec, delta, reason):
    """
    يسجّل حركة نقاط داخل سجل المستخدم نفسه (آخر 50 حركة) لمعرفة سبب أي رصيد لاحقاً.
    يُستدعى قبل حفظ user_rec مباشرة (لا يحفظ بنفسه، فقط يعدّل القاموس في الذاكرة).
    """
    user_rec.setdefault("points_log", [])
    user_rec["points_log"].append({
        "ts": time.strftime("%Y-%m-%d %H:%M", time.gmtime()),
        "delta": delta,
        "reason": reason,
        "balance": user_rec.get("points", 0),
    })
    if len(user_rec["points_log"]) > 50:
        user_rec["points_log"] = user_rec["points_log"][-50:]


def broadcast_target_users(db, seg_key):
    """
    يرجع قائمة آيديات المستخدمين المطابقين لفئة الإعلان المستهدفة:
    'all' لكل المستخدمين، 'lang:ar'/'lang:en' حسب لغة المستخدم، 'country:XX' حسب دولته.
    """
    users_data = load_users()
    all_users = users_data.get("users", {})
    targets = []
    for uid_str, u_rec in all_users.items():
        if seg_key == "all":
            matched = True
        elif seg_key.startswith("lang:"):
            matched = u_rec.get("language") == seg_key[len("lang:"):]
        elif seg_key.startswith("country:"):
            matched = u_rec.get("country") == seg_key[len("country:"):]
        else:
            matched = True
        if matched:
            try:
                targets.append(int(uid_str))
            except ValueError:
                pass
    return targets
...
def process_start(user_obj, args):
    ...
    if db.get("welcome_active", True) and not user_rec.get("welcome_bonus_received", False):
        ...
        log_points_change(user_rec, bonus_given, "هدية التسجيل الأولى")
    ...
    user_rec["inactivity_reminded"] = False
...
def check_and_reward_referral(referred_uid_str):
    """
    يمنح مكافأة الإحالة للمُحيل فقط بعد أن يُظهر "المُحال" نشاطاً حقيقياً
    (مرور يوم كامل على تسجيله، أو فتحه لأي خدمة مقفولة) — حماية من تزوير
    الإحالة عبر حسابات وهمية تدخل وتخرج فوراً بدون أي استخدام حقيقي.
    يرجع True إذا مُنحت المكافأة الآن.
    """
    db = load_db()
    if not db.get("ref_active", True):
        return False

    user_rec = get_user(referred_uid_str)
    if not user_rec:
        return False

    referrer_id = user_rec.get("referred_by")
    if not referrer_id or user_rec.get("referral_rewarded", False):
        return False

    eligible = bool(user_rec.get("unlocked"))
    if not eligible:
        joined = user_rec.get("joined_date")
        if joined:
            try:
                joined_date = datetime.datetime.strptime(joined, "%Y-%m-%d").date()
                today = datetime.datetime.strptime(time.strftime("%Y-%m-%d", time.gmtime()), "%Y-%m-%d").date()
                if (today - joined_date).days >= 1:
                    eligible = True
            except ValueError:
                pass
    if not eligible:
    ...
    log_points_change(referrer_rec, ref_points, f"مكافأة إحالة مستخدم نشط ({referred_uid_str})")
    save_user(referrer_id_str, referrer_rec)

    user_rec["referral_rewarded"] = True
    save_user(referred_uid_str, user_rec)

    ref_name = db.get("ref_name", "نظام الإحالة")
    try:
        bot.send_message(
            int(referrer_id),
            f"🎉 **مبروك!** أحد المستخدمين الذين دخلوا عبر رابط إحالتك (`{referred_uid_str}`) أصبح نشطاً فعلياً في البوت.\n"
...
def run_pending_referral_rewards():
    """يفحص يومياً كل الإحالات المعلّقة (مرّ عليها يوم كامل) ويمنح المكافأة المستحقة."""
    users_data = load_users()
    for uid_str, u_rec in users_data.get("users", {}).items():
        if u_rec.get("referred_by") and not u_rec.get("referral_rewarded", False):
            try:
                check_and_reward_referral(uid_str)
            except Exception:
                logger.exception("Error checking pending referral reward for %s", uid_str)
...
def process_referral_reward(user_id):
    pass  # تم استبدالها بـ check_and_reward_referral (يمنح المكافأة بعد نشاط حقيقي فقط)
...
def claim_daily_gift(user_id):
    ...
    log_points_change(user, current_gift_points, "الهدية اليومية")
    save_user(uid, user)
...
def build_admin_settings_markup(db, parent_id=None):
    ...
    if parent_id is not None:
        ...
        markup.add(types.InlineKeyboardButton("🔍 بحث عن زر بالاسم", callback_data="adm_btn_search_prompt"))
...
def render_inactivity_menu(cid, mid):
    """يعرض شاشة إدارة تذكير المستخدمين الغائبين (يعدّل مكان الرسالة إن توفّر mid، وإلا يرسل رسالة جديدة)."""
    db = load_db()
    active = db.get("inactivity_active", False)
    days = db.get("inactivity_days", 7)
    bonus = db.get("inactivity_bonus_points", 1)
    text = db.get("inactivity_text", DEFAULT_CONFIG["inactivity_text"])

    users_data = load_users()
    today = datetime.datetime.strptime(time.strftime("%Y-%m-%d", time.gmtime()), "%Y-%m-%d").date()
    inactive_count = 0
    for u_rec in users_data.get("users", {}).values():
        lv = u_rec.get("last_visit_date")
        if not lv:
            continue
        try:
            d = (today - datetime.datetime.strptime(lv, "%Y-%m-%d").date()).days
            if d >= days:
                inactive_count += 1
        except ValueError:
            pass
    ...
    markup.add(types.InlineKeyboardButton(
        f"{'✅ مفعّل' if active else '❌ معطّل'} — اضغط للتبديل", callback_data="toggle_inactivity_active"
    ))
    markup.add(
        types.InlineKeyboardButton(f"📅 عدد أيام الغياب: {days}", callback_data="adm_edit_inactivity_days"),
        types.InlineKeyboardButton(f"🎁 نقاط المكافأة: {bonus}", callback_data="adm_edit_inactivity_bonus")
    )
    markup.add(types.InlineKeyboardButton("✏️ تعديل نص التذكير", callback_data="adm_edit_inactivity_text"))
    markup.add(types.InlineKeyboardButton("🚀 تشغيل الفحص الآن يدوياً", callback_data="adm_run_inactivity_now"))
    markup.add(types.InlineKeyboardButton("🔙 رجوع لوحة التحكم", callback_data="adm_back_main"))

    body = (
        "😴 **تذكير المستخدمين الغائبين:**\n\n"
        f"👥 عدد الغائبين حالياً (≥ {days} يوم): **{inactive_count}** مستخدم\n\n"
        f"📩 نص التذكير الحالي:\n«{escape_markdown(text)}»\n\n"
        "الفحص يعمل تلقائياً مرة كل يوم عند التفعيل."
    )
    if mid:
        bot.edit_message_text(body, cid, mid, reply_markup=markup, parse_mode="Markdown")
    else:
        bot.send_message(cid, body, reply_markup=markup, parse_mode="Markdown")
...
# الأزرار التي تبقى في المستوى الرئيسي للوحة التحكم مباشرة (كل ما يخص إدارة الأزرار/الخدمات نفسها)
ADMIN_TOP_LEVEL_BUTTONS = [
...
# باقي الإعدادات مُجمَّعة كل مجموعة متشابهة في قسم خاص بها
ADMIN_CATEGORIES = {
    "rewards": {
        "label": "🎁 إعدادات المكافآت والنقاط",
        "items": [
    ...
    "users": {
        ...
        "items": [
            ...
            ("👥 إدارة المساعدين", "adm_assistants_menu"),
            ("😴 تذكير المستخدمين الغائبين", "adm_inactivity_menu"),
            ("🛡 إدارة الاشتراك الإجباري", "adm_feat_sub"),
        ],
    },
    "stats": {
        "label": "📊 الإحصائيات والنسخ الاحتياطي",
        "items": [
            ("📊 إحصائيات البوت", "adm_stats"),
            ("🏆 لوحة صدارة النقاط", "adm_leaderboard"),
            ("💾 نسخة احتياطية", "adm_backup"),
        ],
    },
    "comms": {
...
ADMIN_PANEL_BUTTONS = ADMIN_TOP_LEVEL_BUTTONS + [(cat["label"], f"adm_cat_{key}") for key, cat in ADMIN_CATEGORIES.items()]
...
def support_command(message):
    ...
    if not is_support_staff(uid):
        return
    clear_state(uid)
    db = load_db()
    messages = db.get("support_messages", [])
    days = {}
    for m in messages:
    ...
    for day in sorted(days.keys(), reverse=True):
        day_msgs = days[day]
        senders = len(set(m["user_id"] for m in day_msgs))
        markup.add(types.InlineKeyboardButton(
            f"📅 {day} — {len(day_msgs)} رسالة من {senders} مستخدم", callback_data=f"adm_support_day_{day}"
        ))

    body = (
        f"📥 **صندوق الرسائل الواردة (تواصل الدعم):**\n\nإجمالي الأيام: {len(days)}\n\nاختر يوماً لعرض رسائله:"
        if days else
        "📥 **صندوق الرسائل الواردة (تواصل الدعم):**\n\nلا توجد أي رسائل مستلمة حتى الآن."
    )
    bot.send_message(message.chat.id, body, reply_markup=markup, parse_mode="Markdown")
...
def handle_dynamic_admin_actions(call):
    ...
    if data.startswith("adm_edit_"):
        ...
        markup.add(
            ...
            types.InlineKeyboardButton("🗑️ حذف الخدمة/الزر", callback_data=f"del_confirm_{btn_id}")
        )
...
def callback(call):
    ...
    try:
        ...
        if data == "adm_broadcast_menu":
            ...
            markup = types.InlineKeyboardMarkup(row_width=1)
            markup.add(types.InlineKeyboardButton("🌍 جميع المستخدمين", callback_data="bc_seg_all"))
            markup.add(types.InlineKeyboardButton("🇸🇦 مختاري العربية فقط", callback_data="bc_seg_lang:ar"))
            markup.add(types.InlineKeyboardButton("🇬🇧 مختاري الإنجليزية فقط", callback_data="bc_seg_lang:en"))
            markup.add(types.InlineKeyboardButton("🌎 دولة معيّنة", callback_data="bc_seg_country_prompt"))
            markup.add(types.InlineKeyboardButton("🔙 إلغاء", callback_data="adm_broadcast_menu"))
            bot.edit_message_text("📣 **الخطوة 1 من 3:** اختر الفئة المستهدفة بالإعلان:", cid, mid, reply_markup=markup, parse_mode="Markdown")
            return

        elif data == "bc_seg_country_prompt":
            set_state(uid, "WAIT_BROADCAST_COUNTRY")
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("🔙 إلغاء", callback_data="adm_start_broadcast"))
            bot.edit_message_text(
                "✍️ أرسل رمز الدولة (مثال: `SA` للسعودية، `EG` لمصر):\n/cancel للإلغاء",
                cid, mid, reply_markup=markup, parse_mode="Markdown"
            )
            return

        elif data.startswith("bc_seg_"):
            seg_key = data[len("bc_seg_"):]
            markup = types.InlineKeyboardMarkup(row_width=1)
            markup.add(types.InlineKeyboardButton("🚀 إرسال الآن", callback_data=f"bc_time_now_{seg_key}"))
            markup.add(types.InlineKeyboardButton("🕒 جدولة لوقت لاحق", callback_data=f"bc_time_sched_{seg_key}"))
            markup.add(types.InlineKeyboardButton("🔙 رجوع", callback_data="adm_start_broadcast"))
            bot.edit_message_text("📣 **الخطوة 2 من 3:** متى تريد الإرسال؟", cid, mid, reply_markup=markup, parse_mode="Markdown")
            return

        elif data.startswith("bc_time_now_"):
            seg_key = data[len("bc_time_now_"):]
            db = load_db()
            targets = broadcast_target_users(db, seg_key)
            set_state(uid, WAIT_BROADCAST, segment=seg_key, scheduled_at=None)
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("🔙 إلغاء البث", callback_data="adm_broadcast_menu"))
            bot.edit_message_text(
                f"📣 **الخطوة 3 من 3:** أرسل الآن رسالة الإعلان (سيصل لـ {len(targets)} مستخدم مطابق للفئة المختارة):\n\n"
                f"(يمكنك إرسال نص، صورة، فيديو، ملف، صوت... وسيتم تطبيق إعدادات التثبيت والإشعار الحالية)\n\n/cancel للإلغاء",
                cid, mid, reply_markup=markup, parse_mode="Markdown"
            )
            return

        elif data.startswith("bc_time_sched_"):
            seg_key = data[len("bc_time_sched_"):]
            set_state(uid, WAIT_BROADCAST_SCHEDULE_TIME, segment=seg_key)
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("🔙 إلغاء البث", callback_data="adm_broadcast_menu"))
            bot.edit_message_text(
                "🕒 أرسل الآن وقت الإرسال المطلوب بتوقيت غرينتش UTC، بالصيغة:\n`YYYY-MM-DD HH:MM`\n"
                "مثال: `2026-08-15 18:30`\n\n/cancel للإلغاء",
                cid, mid, reply_markup=markup, parse_mode="Markdown"
            )
            return
        ...
        if data == "adm_gift_codes_menu":
            ...
            if not codes:
                ...
                return
            for c in codes:
                markup.add(types.InlineKeyboardButton(f"🗑️ حذف الكود: {c['code']} ({c['points']} نقطة)", callback_data=f"del_gcode_confirm_{c['code']}"))
            markup.add(types.InlineKeyboardButton("🔙 رجوع", callback_data="adm_gift_codes_menu"))
            bot.edit_message_text("اختر الكود المراد حذفه نهائياً:", cid, mid, reply_markup=markup)
            return

        elif data.startswith("del_gcode_confirm_"):
            target_code = data[len("del_gcode_confirm_"):]
            markup = types.InlineKeyboardMarkup()
            markup.add(
                types.InlineKeyboardButton("✅ نعم، احذف", callback_data=f"del_gcode_{target_code}"),
                types.InlineKeyboardButton("❌ إلغاء", callback_data="adm_del_gift_code_prompt")
            )
            bot.edit_message_text(
                f"⚠️ متأكد من حذف الكود «{escape_markdown(target_code)}» نهائياً؟",
                cid, mid, reply_markup=markup, parse_mode="Markdown"
            )
            return
        ...
        # ── إدارة مهام النقاط (انضمام لقناة/مجموعة/بوت مقابل نقاط) ──
            ...
            markup.add(types.InlineKeyboardButton("🗑️ حذف هذه المهمة نهائياً", callback_data=f"adm_task_delete_confirm_prompt_{task_id}"))
        ...
        elif data.startswith("adm_task_delete_confirm_prompt_"):
            task_id = data[len("adm_task_delete_confirm_prompt_"):]
            db = load_db()
            task = next((t for t in db.get("tasks", []) if t["id"] == task_id), None)
            if not task:
                bot.answer_callback_query(call.id, "⚠️ هذه المهمة لم تعد موجودة.", show_alert=True)
            ...
            markup = types.InlineKeyboardMarkup()
            markup.add(
                types.InlineKeyboardButton("✅ نعم، احذف نهائياً", callback_data=f"adm_task_delete_{task_id}"),
                types.InlineKeyboardButton("❌ إلغاء", callback_data=f"adm_task_view_{task_id}")
            )
            bot.edit_message_text(
                f"⚠️ **متأكد من حذف المهمة «{escape_markdown(task['name'])}» نهائياً؟**\n\nهذا الإجراء لا يمكن التراجع عنه.",
                cid, mid, reply_markup=markup, parse_mode="Markdown"
            )
            return
        ...
        if data == "adm_btn_search_prompt":
            set_state(uid, WAIT_BTN_SEARCH)
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("🔙 إلغاء", callback_data="adm_settings_list"))
            bot.edit_message_text(
                "🔍 أرسل الآن كل أو جزء من اسم الزر/الخدمة/القسم الذي تبحث عنه:\n/cancel للإلغاء",
        ...
        if data == "adm_assistants_menu":
            db = load_db()
            assistants = db.get("assistant_admins", [])
            markup = types.InlineKeyboardMarkup(row_width=1)
            for a in assistants:
                markup.add(types.InlineKeyboardButton(
                    f"❌ إزالة: {a.get('name', 'مساعد')} ({a['id']})", callback_data=f"adm_assistant_remove_{a['id']}"
                ))
            markup.add(types.InlineKeyboardButton("➕ إضافة مساعد جديد", callback_data="adm_assistant_add_prompt"))
            markup.add(types.InlineKeyboardButton("🔙 رجوع لوحة التحكم", callback_data="adm_back_main"))

            body = (
                "👥 **إدارة المساعدين:**\n\n"
                "المساعد يقدر يستخدم أمر `/support` فقط لعرض والرد على رسائل تواصل الدعم — "
                "بدون أي وصول لباقي إعدادات البوت.\n\n"
                f"عدد المساعدين الحاليين: {len(assistants)}"
            )
            bot.edit_message_text(body, cid, mid, reply_markup=markup, parse_mode="Markdown")
            return

        if data == "adm_assistant_add_prompt":
            set_state(uid, WAIT_ASSISTANT_ADD_ID)
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("🔙 إلغاء", callback_data="adm_assistants_menu"))
            bot.edit_message_text(
                "✍️ أرسل الآن آيدي (ID) المستخدم الذي تريد إضافته كمساعد للرد على الدعم:\n/cancel للإلغاء",
                cid, mid, reply_markup=markup
            )
            return

        if data.startswith("adm_assistant_remove_"):
            target_id = int(data[len("adm_assistant_remove_"):])
            db = load_db()
            db["assistant_admins"] = [a for a in db.get("assistant_admins", []) if a["id"] != target_id]
            save_db(db)
            bot.answer_callback_query(call.id, "✅ تم إزالة المساعد بنجاح!")

            assistants = db["assistant_admins"]
            markup = types.InlineKeyboardMarkup(row_width=1)
            for a in assistants:
                markup.add(types.InlineKeyboardButton(
                    f"❌ إزالة: {a.get('name', 'مساعد')} ({a['id']})", callback_data=f"adm_assistant_remove_{a['id']}"
            ...
            markup.add(types.InlineKeyboardButton("🔙 رجوع لوحة التحكم", callback_data="adm_back_main"))
            bot.edit_message_text(
                f"👥 **إدارة المساعدين:**\n\nعدد المساعدين الحاليين: {len(assistants)}",
                cid, mid, reply_markup=markup, parse_mode="Markdown"
            )
            return

        if data == "adm_inactivity_menu":
            render_inactivity_menu(cid, mid)
            return

        if data == "toggle_inactivity_active":
            db = load_db()
            db["inactivity_active"] = not db.get("inactivity_active", False)
            save_db(db)
            bot.answer_callback_query(call.id, "✅ تم تحديث الحالة!")
            render_inactivity_menu(cid, mid)
            return

        if data == "adm_run_inactivity_now":
            bot.answer_callback_query(call.id, "⏳ جاري الفحص والإرسال...")
            db = load_db()
            db["inactivity_last_run_date"] = None
            save_db(db)
            sent = run_inactivity_check()
            bot.send_message(cid, f"✅ تم الفحص، وأُرسل التذكير إلى {sent} مستخدم غائب.")
            render_inactivity_menu(cid, None)
            return

        if data == "adm_edit_inactivity_days":
            set_state(uid, WAIT_INACTIVITY_DAYS)
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("🔙 إلغاء", callback_data="adm_inactivity_menu"))
            bot.edit_message_text("✍️ أرسل عدد أيام الغياب المطلوب لاعتبار المستخدم غائباً (رقم صحيح):", cid, mid, reply_markup=markup)
            return

        if data == "adm_edit_inactivity_bonus":
            set_state(uid, WAIT_INACTIVITY_DAYS, editing_bonus=True)
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("🔙 إلغاء", callback_data="adm_inactivity_menu"))
            bot.edit_message_text("✍️ أرسل عدد نقاط المكافأة عند التذكير (0 لإلغاء المكافأة):", cid, mid, reply_markup=markup)
            return

        if data == "adm_edit_inactivity_text":
            set_state(uid, WAIT_INACTIVITY_TEXT)
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("🔙 إلغاء", callback_data="adm_inactivity_menu"))
            bot.edit_message_text("✍️ أرسل نص التذكير الجديد الذي سيصل للمستخدمين الغائبين:", cid, mid, reply_markup=markup)
            return

        if data == "adm_leaderboard":
            users_data = load_users()
            all_users = users_data.get("users", {})
            sorted_by_points = sorted(all_users.items(), key=lambda item: item[1].get("points", 0), reverse=True)
            medals = ["🥇", "🥈", "🥉", "🎖", "🎖"]
            lines = []
            for idx, (u_id, u_info) in enumerate(sorted_by_points[:15]):
                medal = medals[idx] if idx < len(medals) else "🏅"
                lines.append(f"{medal} {escape_markdown(u_info.get('name', 'بدون اسم'))} (`{u_id}`) — {u_info.get('points', 0)} نقطة")
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("🔙 رجوع لوحة التحكم", callback_data="adm_back_main"))
            body = "🏆 **لوحة صدارة النقاط (أعلى 15):**\n\n" + ("\n".join(lines) if lines else "لا يوجد مستخدمون بعد.")
            bot.edit_message_text(body, cid, mid, reply_markup=markup, parse_mode="Markdown")
            return

        if data == "adm_backup":
            bot.answer_callback_query(call.id, "⏳ جاري تجهيز النسخة الاحتياطية...")
            db = load_db()
            users_data = load_users()
            backup = {
                "exported_at": time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime()) + " UTC",
                "config": db,
            ...
            try:
                ...
                file_obj.name = f"backup_{time.strftime('%Y%m%d_%H%M%S', time.gmtime())}.json"
                bot.send_document(
                    cid, file_obj,
                    caption=f"💾 نسخة احتياطية كاملة للبوت\n📅 {backup['exported_at']}\n👥 عدد المستخدمين: {len(backup['users'])}\n🔘 عدد الأزرار: {len(db.get('buttons', []))}"
                )
            except Exception as e:
                logger.exception("Backup export failed: %s", e)
                bot.send_message(cid, "⚠️ حدث خطأ أثناء تجهيز النسخة الاحتياطية.")
            return
        ...
        if data == "adm_support_settings":
            ...
            support_messages = db.get("support_messages", [])
            total_msgs = len(support_messages)
            distinct_senders = len(set(m["user_id"] for m in support_messages))

            markup = types.InlineKeyboardMarkup(row_width=1)
            markup.add(types.InlineKeyboardButton(
                f"📥 صندوق الرسائل الواردة ({total_msgs})", callback_data="adm_support_inbox"
            ))
            if support_btn:
                markup.add(types.InlineKeyboardButton(
                    f"⚙️ فتح إدارة الزر المفعّل: «{escape_markdown(support_btn['name'])}»", callback_data=f"adm_set_click_{support_btn_id}"
                ))
            markup.add(types.InlineKeyboardButton("✏️ تعديل رسالة الاستلام (الرد التلقائي)", callback_data="adm_edit_support_text"))
            markup.add(types.InlineKeyboardButton("🔙 رجوع لوحة التحكم", callback_data="adm_back_main"))

            status = (
                f"☎️ الزر المفعّل حالياً: «{escape_markdown(support_btn['name'])}»" if support_btn
                else "⚠️ لم يتم تفعيل أي زر كـ (تواصل مع الدعم) بعد."
            )
            bot.edit_message_text(
                "☎️ **إعدادات تواصل الدعم:**\n\n"
                f"{status}\n\n"
                f"📊 إجمالي الرسائل المستلمة: {total_msgs} من {distinct_senders} مستخدم\n\n"
                "📩 رسالة الاستلام الحالية (تُرسل تلقائياً لأي مستخدم بعد إرسال رسالته):\n"
                f"«{escape_markdown(db.get('support_received_text', DEFAULT_CONFIG['support_received_text']))}»\n\n"
        ...
        if data == "adm_support_inbox":
            ...
            markup.add(types.InlineKeyboardButton("🔙 رجوع", callback_data="adm_support_settings"))

            body = (
                f"📥 **صندوق الرسائل الواردة:**\n\nإجمالي الأيام: {len(days)}\n\nاختر يوماً لعرض رسائله:"
                if days else
        ...
        if data.startswith("adm_support_day_"):
            day = data[len("adm_support_day_"):]
            db = load_db()
            messages = db.get("support_messages", [])
            day_msgs = [m for m in messages if m["date"] == day]
            day_msgs.sort(key=lambda m: m.get("timestamp", 0), reverse=True)

            markup = types.InlineKeyboardMarkup(row_width=1)
            for m in day_msgs:
                preview = (m.get("text") or f"[{m.get('content_type', 'محتوى')}]").strip().replace("\n", " ")
                if len(preview) > 35:
                    preview = preview[:35] + "…"
                markup.add(types.InlineKeyboardButton(
                    f"👤 {m['name']}: {preview}", callback_data=f"adm_support_msg_{m['id']}"
                ))
            markup.add(types.InlineKeyboardButton("🔙 رجوع لصندوق الرسائل", callback_data="adm_support_inbox"))

            senders = len(set(m["user_id"] for m in day_msgs))
            bot.edit_message_text(
                f"📅 **رسائل يوم {day}:**\n\nعدد الرسائل: {len(day_msgs)} من {senders} مستخدم\n\nاختر رسالة لعرضها كاملة:",
                cid, mid, reply_markup=markup, parse_mode="Markdown"
            )
            return

        if data.startswith("adm_support_msg_"):
            msg_id = data[len("adm_support_msg_"):]
            db = load_db()
            messages = db.get("support_messages", [])
            support_msg = next((m for m in messages if m["id"] == msg_id), None)
            if not support_msg:
                bot.answer_callback_query(call.id, "⚠️ هذه الرسالة لم تعد موجودة.", show_alert=True)
                return

            username_line = f"@{support_msg['username']}" if support_msg.get("username") else "(لا يوجد يوزر)"
            header = (
                f"📩 **رسالة بتاريخ {support_msg['date']}:**\n\n"
                f"👤 الاسم: {escape_markdown(support_msg['name'])}\n"
                f"📛 اليوزر: {escape_markdown(username_line)}\n"
                f"🆔 الآيدي: [{support_msg['user_id']}](tg://user?id={support_msg['user_id']})\n\n"
                "━━━━━━━━━━\n"
            )

            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("🔙 رجوع لرسائل هذا اليوم", callback_data=f"adm_support_day_{support_msg['date']}"))
            ...
            ct = support_msg.get("content_type", "text")
            file_id = support_msg.get("file_id")
            text_body = support_msg.get("text") or ""

            try:
                if ct == "text" or not file_id:
                    bot.send_message(cid, header + escape_markdown(text_body), reply_markup=markup, parse_mode="Markdown")
                elif ct == "photo":
                    bot.send_photo(cid, file_id, caption=header + escape_markdown(text_body), reply_markup=markup, parse_mode="Markdown")
                elif ct == "document":
                    bot.send_document(cid, file_id, caption=header + escape_markdown(text_body), reply_markup=markup, parse_mode="Markdown")
                elif ct == "video":
                    bot.send_video(cid, file_id, caption=header + escape_markdown(text_body), reply_markup=markup, parse_mode="Markdown")
                elif ct == "voice":
                    bot.send_voice(cid, file_id, caption=header + escape_markdown(text_body), reply_markup=markup, parse_mode="Markdown")
                elif ct == "audio":
                    bot.send_audio(cid, file_id, caption=header + escape_markdown(text_body), reply_markup=markup, parse_mode="Markdown")
                elif ct == "sticker":
                    bot.send_message(cid, header + "(ملصق 🎨)", parse_mode="Markdown")
                    bot.send_sticker(cid, file_id, reply_markup=markup)
                else:
                    bot.send_message(cid, header + escape_markdown(text_body), reply_markup=markup, parse_mode="Markdown")
            except Exception as e:
                logger.exception("Failed to render support message detail: %s", e)
                bot.send_message(cid, header + "⚠️ تعذّر عرض محتوى هذه الرسالة.", reply_markup=markup, parse_mode="Markdown")
            return
        ...
        if data == "my_info":
            ...
            info_markup.add(types.InlineKeyboardButton(translate_text("🏆 لوحة صدارة النقاط", lang), callback_data="points_leaderboard"))
        ...
        if data == "points_leaderboard":
            ...
            medals = ["🥇", "🥈", "🥉", "🎖", "🎖"]
            lines = []
            for idx, (u_id, u_info) in enumerate(sorted_by_points[:10]):
                medal = medals[idx] if idx < len(medals) else "🏅"
                marker = " ⬅️" if u_id == uid_str else ""
                lines.append(f"{medal} {escape_markdown(u_info.get('name', 'بدون اسم'))} — {u_info.get('points', 0)} نقطة{marker}")

            my_rank = next((i + 1 for i, (u_id, _) in enumerate(sorted_by_points) if u_id == uid_str), len(sorted_by_points))
            my_points = all_users.get(uid_str, {}).get("points", 0)

            body = (
                translate_text("🏆 **لوحة صدارة النقاط (أعلى 10):**", lang) + "\n\n"
                + ("\n".join(lines) if lines else translate_text("لا يوجد مستخدمون بعد.", lang))
                + "\n\n" + translate_text(f"📍 ترتيبك أنت: المركز {my_rank} برصيد {my_points} نقطة", lang)
            )
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton(t("back", lang), callback_data="my_info"))
            edit_or_replace(cid, mid, call.message.content_type == "text", body, markup=markup, parse_mode="Markdown")
            return
        ...
        if data.startswith("task_check_"):
            ...
            log_points_change(user_rec, int(task["points"]), f"إكمال مهمة: {task['name']}")
        ...
        if data.startswith("adm_cat_"):
            cat_key = data[len("adm_cat_"):]
            category = ADMIN_CATEGORIES.get(cat_key)
            if not category:
                bot.answer_callback_query(call.id, "⚠️ هذا القسم غير موجود.", show_alert=True)
                return
            markup = types.InlineKeyboardMarkup()
        ...
        if data.startswith("pay_"):
            ...
            log_points_change(user, -unlock_pts, f"فتح خدمة: {btn.get('name', btn_id)}")
            save_user(uid_str, user)
            try:
                check_and_reward_referral(uid_str)
            except Exception:
                logger.exception("Error checking referral reward after unlock for %s", uid_str)
        ...
        if data.startswith("cpick_"):
            ...
            if not user_rec.get("country_bonus_given"):
                ...
                log_points_change(user_rec, bonus_pts, "مكافأة اختيار الدولة")
        ...
        if data == "adm_add":
            ...
            for btn in db["buttons"]:
                ...
                markup.add(
                    types.InlineKeyboardButton(label, callback_data=f"del_confirm_{btn['id']}")
                )
            bot.send_message(
            ...
            extra = f"\n\n⚠️ سيُحذف معه أيضاً **{len(descendants)}** زر/خدمة فرعية بداخله!" if descendants else ""
            markup = types.InlineKeyboardMarkup()
            markup.add(
                types.InlineKeyboardButton("✅ نعم، احذف نهائياً", callback_data=f"del_{btn_id}"),
                types.InlineKeyboardButton("❌ إلغاء", callback_data="adm_delete")
            )
            bot.send_message(
                cid,
                f"⚠️ **متأكد من حذف «{escape_markdown(btn['name'])}» نهائياً؟**{extra}\n\nهذا الإجراء لا يمكن التراجع عنه.",
        ...
        # ── إدارة المستخدمين المحدثة بالكامل ──
            ...
            if action in ["add", "sub"]:
                ...
                old_pts = user_rec.get("points", 0)
                user_rec["points"] = 0
                log_points_change(user_rec, -old_pts, "تصفير الرصيد من الأدمن")
                ...
                log_entries = user_rec.get("points_log", [])
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton("🔙 رجوع", callback_data="adm_users"))
                if not log_entries:
                    bot.edit_message_text(
                        f"📜 لا توجد أي حركة نقاط مسجَّلة بعد للمستخدم (`{target_id}`).",
                        cid, mid, reply_markup=markup, parse_mode="Markdown"
                    )
                    return
                lines = []
                for entry in reversed(log_entries[-20:]):
                    ...
                    lines.append(f"🕐 {entry['ts']} — {sign}{entry['delta']} ({escape_markdown(entry['reason'])}) → الرصيد: {entry['balance']}")
                bot.edit_message_text(
                    f"📜 **سجل حركة النقاط للمستخدم (`{target_id}`)** — آخر {len(lines)} حركة:\n\n" + "\n".join(lines),
                    cid, mid, reply_markup=markup, parse_mode="Markdown"
                )
                return
...
def handle_state(message):
    ...
    if state == WAIT_NEW_CODE_STR:
        ...
        payload_content_type = message.content_type
        payload_text = message.text if payload_content_type == "text" else (message.caption or "")
        payload_file_id = None
        if payload_content_type == "photo":
            payload_file_id = message.photo[-1].file_id
        elif payload_content_type == "document":
            payload_file_id = message.document.file_id
        elif payload_content_type == "video":
            payload_file_id = message.video.file_id
        elif payload_content_type == "voice":
            payload_file_id = message.voice.file_id
        elif payload_content_type == "audio":
            payload_file_id = message.audio.file_id
        elif payload_content_type == "sticker":
            payload_file_id = message.sticker.file_id

        db = load_db()
        db.setdefault("support_messages", [])
        db["support_messages"].append({
            "id": new_id(),
        ...
        db = load_db()
        matches = [b for b in db.get("buttons", []) if query in b.get("name", "").lower()]

        markup = types.InlineKeyboardMarkup(row_width=1)
        if matches:
            for b in matches[:25]:
                has_sub = len(get_children(db, b["id"])) > 0
                icon = "📁" if has_sub else "📄"
                lock_icon = " 🔒" if int(b.get("unlock_points", 0)) > 0 else ""
                markup.add(types.InlineKeyboardButton(f"{icon} {b['name']}{lock_icon}", callback_data=f"adm_set_click_{b['id']}"))
        markup.add(types.InlineKeyboardButton("🔙 رجوع لإعدادات الأزرار", callback_data="adm_settings_list"))

        body = (
            f"🔍 نتائج البحث عن «{message.text.strip()}»: ({len(matches)} نتيجة)\n\nاضغط على أي نتيجة لإدارتها:"
            if matches else
            f"🔍 لا توجد أي نتائج تطابق «{message.text.strip()}»."
        )
        bot.send_message(cid, body, reply_markup=markup)
        return

    elif state == WAIT_INACTIVITY_DAYS:
        if message.content_type != "text":
            bot.send_message(cid, "⚠️ أرسل رقماً صحيحاً.")
            return
        try:
            val = int(message.text.strip())
        except ValueError:
            bot.send_message(cid, "❌ خطأ: أرسل رقماً صحيحاً فقط.")
            return
        editing_bonus = get_data(uid).get("editing_bonus", False)
        db = load_db()
        if editing_bonus:
            db["inactivity_bonus_points"] = max(0, val)
            confirm = f"✅ تم تحديث نقاط مكافأة التذكير إلى: {max(0, val)} نقطة."
        else:
            if val < 1:
                bot.send_message(cid, "❌ يجب أن يكون عدد الأيام رقماً موجباً.")
                return
            db["inactivity_days"] = val
            confirm = f"✅ تم تحديث عدد أيام الغياب إلى: {val} يوم."
        save_db(db)
        clear_state(uid)
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("🔙 عودة", callback_data="adm_inactivity_menu"))
        bot.send_message(cid, confirm, reply_markup=markup)
        return

    elif state == WAIT_INACTIVITY_TEXT:
        if message.content_type != "text":
            bot.send_message(cid, "⚠️ أرسل النص كرسالة نصية من فضلك.")
            return
        new_text = message.text.strip()
        db = load_db()
        db["inactivity_text"] = new_text
        save_db(db)
        clear_state(uid)
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("🔙 عودة", callback_data="adm_inactivity_menu"))
        bot.send_message(cid, f"✅ تم تحديث نص التذكير:\n\n«{new_text}»", reply_markup=markup)
        return

    elif state == WAIT_ASSISTANT_ADD_ID:
        if message.content_type != "text":
            bot.send_message(cid, "⚠️ أرسل الآيدي كرقم من فضلك.")
            return
        try:
            new_assistant_id = int(message.text.strip())
        except ValueError:
            bot.send_message(cid, "❌ خطأ: أرسل آيدي رقمي صحيح فقط.")
            return

        clear_state(uid)
        db = load_db()
        db.setdefault("assistant_admins", [])
        if any(a["id"] == new_assistant_id for a in db["assistant_admins"]):
            bot.send_message(cid, "⚠️ هذا المستخدم مساعد بالفعل.")
            return

        target_user_rec = get_user(str(new_assistant_id))
        assistant_name = target_user_rec.get("name", "مساعد") if target_user_rec else "مساعد"
        db["assistant_admins"].append({"id": new_assistant_id, "name": assistant_name})
        save_db(db)

        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("🔙 عودة لإدارة المساعدين", callback_data="adm_assistants_menu"))
        bot.send_message(
            cid,
            f"✅ **تمت إضافة المساعد بنجاح!**\n\n"
            f"• الآيدي: `{new_assistant_id}`\n"
            f"• الاسم: {escape_markdown(assistant_name)}\n\n"
            f"يقدر الآن يستخدم أمر /support للرد على رسائل تواصل الدعم فقط.",
            reply_markup=markup,
            parse_mode="Markdown"
        )
        try:
            bot.send_message(
                new_assistant_id,
                "🎉 تمت إضافتك كمساعد للرد على تواصل الدعم في البوت!\n\nاستخدم الأمر /support في أي وقت لعرض الرسائل الواردة."
            )
        except Exception:
            logger.exception("Could not notify new assistant %s", new_assistant_id)
        return
        ...
        user_rec["points"] = user_rec.get("points", 0) + pts
        log_points_change(user_rec, pts, f"استبدال كود هدية ({entered_code})")
        save_user(uid_str, user_rec)
        clear_state(uid)
        ...
        bot.send_message(
            ...
            f"• رصيدك الحالي: **{user_rec['points']}** نقطة 🌟",
            parse_mode="Markdown"
        )
        ...
        try:
            ...
            markup.add(types.InlineKeyboardButton("📜 سجل حركة النقاط", callback_data=f"usropt_log_{target_id}"))
            markup.add(types.InlineKeyboardButton("🔙 رجوع لإدارة المستخدمين", callback_data="adm_users"))
        ...
        try:
            ...
            if action == "add":
                ...
                log_points_change(user_rec, amount, "تعديل يدوي من الأدمن (إضافة)")
                ...
                actual_delta = new_balance - current_pts
                user_rec["points"] = new_balance
                log_points_change(user_rec, actual_delta, "تعديل يدوي من الأدمن (خصم)")
    ...
    elif state == "WAIT_BROADCAST_COUNTRY":
        if message.content_type != "text":
            bot.send_message(cid, "⚠️ أرسل رمز الدولة كنص من فضلك.")
            return
        code = message.text.strip().upper()
        if code not in COUNTRY_LOOKUP:
            bot.send_message(cid, "❌ رمز دولة غير معروف. أرسل رمزاً صحيحاً (مثال: `SA`، `EG`، `AE`).", parse_mode="Markdown")
            return
        clear_state(uid)
        seg_key = f"country:{code}"
        markup = types.InlineKeyboardMarkup(row_width=1)
        markup.add(types.InlineKeyboardButton("🚀 إرسال الآن", callback_data=f"bc_time_now_{seg_key}"))
        markup.add(types.InlineKeyboardButton("🕒 جدولة لوقت لاحق", callback_data=f"bc_time_sched_{seg_key}"))
        markup.add(types.InlineKeyboardButton("🔙 رجوع", callback_data="adm_start_broadcast"))
        bot.send_message(
            cid,
            f"✅ الفئة المختارة: {flag(code)} {COUNTRY_LOOKUP[code]}\n\n📣 متى تريد الإرسال؟",
            reply_markup=markup
        )
        return

    elif state == WAIT_BROADCAST_SCHEDULE_TIME:
        if message.content_type != "text":
            bot.send_message(cid, "⚠️ أرسل الوقت كنص بالصيغة المطلوبة.")
            return
        try:
            dt = datetime.datetime.strptime(message.text.strip(), "%Y-%m-%d %H:%M")
            scheduled_ts = dt.replace(tzinfo=datetime.timezone.utc).timestamp()
        except ValueError:
            bot.send_message(cid, "❌ صيغة غير صحيحة. أرسل الوقت بالشكل: `YYYY-MM-DD HH:MM`", parse_mode="Markdown")
            return
        if scheduled_ts <= time.time():
            bot.send_message(cid, "❌ هذا الوقت في الماضي! أرسل وقتاً مستقبلياً.")
            return

        st_data = get_data(uid)
        segment = st_data.get("segment", "all")
        set_state(uid, WAIT_BROADCAST, segment=segment, scheduled_at=scheduled_ts)
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("🔙 إلغاء البث", callback_data="adm_broadcast_menu"))
        bot.send_message(
            cid,
            f"✅ سيُرسَل بتاريخ: {message.text.strip()} UTC\n\n"
            "📣 أرسل الآن رسالة الإعلان (نص، صورة، فيديو، ملف...):\n/cancel للإلغاء",
            reply_markup=markup
        )
        return
        ...
        st_data = get_data(uid)
        segment = st_data.get("segment", "all")
        scheduled_at = st_data.get("scheduled_at")
        db = load_db()

        if scheduled_at:
            db.setdefault("scheduled_broadcasts", [])
            ...
            save_db(db)
            clear_state(uid)
            targets_count = len(broadcast_target_users(db, segment))
            send_dt = datetime.datetime.fromtimestamp(scheduled_at, tz=datetime.timezone.utc)
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("🔙 عودة لإعدادات الإعلان", callback_data="adm_broadcast_menu"))
            bot.send_message(
                cid,
                f"✅ **تمت جدولة الإعلان بنجاح!**\n\n"
                f"• سيُرسَل بتاريخ: {send_dt.strftime('%Y-%m-%d %H:%M')} UTC\n"
                f"• عدد المستلمين المتوقع: ~{targets_count} مستخدم\n\n"
                f"سأتولى الإرسال تلقائياً في الوقت المحدد.",
                reply_markup=markup,
                parse_mode="Markdown"
            )
            return
        ...
        sent = 0
        failed = 0
        for target_uid in broadcast_target_users(db, segment):
...
def run_inactivity_check():
    """يفحص المستخدمين الغائبين ويرسل لهم تذكيراً (مرة واحدة يومياً كحد أقصى)."""
    db = load_db()
    if not db.get("inactivity_active", False):
        return 0
    today_str = time.strftime("%Y-%m-%d", time.gmtime())
    if db.get("inactivity_last_run_date") == today_str:
        return 0

    days_threshold = int(db.get("inactivity_days", 7))
    bonus = int(db.get("inactivity_bonus_points", 1))
    base_text = db.get("inactivity_text", DEFAULT_CONFIG["inactivity_text"])
    today = datetime.datetime.strptime(today_str, "%Y-%m-%d").date()

    users_data = load_users()
    sent_count = 0
    for uid_str, u_rec in users_data.get("users", {}).items():
        last_visit = u_rec.get("last_visit_date")
        if not last_visit or u_rec.get("inactivity_reminded", False):
            continue
        try:
            last_visit_date = datetime.datetime.strptime(last_visit, "%Y-%m-%d").date()
        except ValueError:
            continue
        if (today - last_visit_date).days < days_threshold:
            continue
        try:
            msg_text = base_text
            if bonus > 0:
                u_rec["points"] = u_rec.get("points", 0) + bonus
                log_points_change(u_rec, bonus, "مكافأة تذكير الغياب")
                msg_text += f"\n\n🎁 حصلت على {bonus} نقطة هدية بمناسبة رجوعك!"
            u_rec["inactivity_reminded"] = True
            save_user(uid_str, u_rec)
            bot.send_message(int(uid_str), msg_text)
            sent_count += 1
        except Exception as e:
            logger.exception("Failed to send inactivity reminder to %s: %s", uid_str, e)

    db = load_db()
    db["inactivity_last_run_date"] = today_str
    save_db(db)
    return sent_count


def run_scheduled_broadcasts():
    """يرسل أي إعلانات مجدولة حان وقتها."""
    db = load_db()
    pending = db.get("scheduled_broadcasts", [])
    due = [b for b in pending if not b.get("sent") and b.get("send_at", 0) <= time.time()]
    if not due:
        return
    ...
    for bc in due:
        targets = broadcast_target_users(db, bc.get("segment", "all"))
        sent, failed = 0, 0
        for target_uid in targets:
            if target_uid == ADMIN_ID:
                continue
            try:
                sent_msg = bot.copy_message(target_uid, bc["from_chat_id"], bc["message_id"], disable_notification=is_silent)
                if should_pin:
                    try:
                        bot.pin_chat_message(target_uid, sent_msg.message_id)
                    except Exception:
                ...
                logger.exception("Failed to send scheduled broadcast to %s", target_uid)
                failed += 1
        bc["sent"] = True
        try:
            bot.send_message(
                ADMIN_ID,
                f"📣 تم إرسال إعلانك المجدوَل بنجاح!\n✅ نجح: {sent} | ❌ فشل: {failed}"
            )
        except Exception:
            logger.exception("Failed to notify admin about completed scheduled broadcast")

    db = load_db()
    db["scheduled_broadcasts"] = [b for b in db.get("scheduled_broadcasts", []) if not b.get("sent") or b["id"] in [d["id"] for d in due]]
    for b in db["scheduled_broadcasts"]:
        if b["id"] in [d["id"] for d in due]:
            b["sent"] = True
    save_db(db)


def background_scheduler_loop():
    """خيط خلفي يفحص الإعلانات المجدولة كل دقيقة، وتذكير الغياب والإحالات المعلّقة كل ساعة."""
    last_inactivity_check = 0
    last_referral_check = 0
    while True:
        try:
            run_scheduled_broadcasts()
        except Exception:
            logger.exception("Error in scheduled broadcast check")
        try:
            if time.time() - last_inactivity_check >= 3600:
                run_inactivity_check()
                last_inactivity_check = time.time()
        except Exception:
            logger.exception("Error in inactivity check")
        try:
            if time.time() - last_referral_check >= 3600:
                run_pending_referral_rewards()
                last_referral_check = time.time()
        except Exception:
            logger.exception("Error in pending referral rewards check")
        time.sleep(60)


scheduler_thread = Thread(target=background_scheduler_loop, daemon=True)
